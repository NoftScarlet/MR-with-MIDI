...
export default class Header extends React.Component {
            ...
            if ((this.scoreSetTemp[i]["ptKeyName"] === event.key) && (!this.keyLock)) {
                ...
                let loopLength = (() => {
                    for (let j = i + 1; j < this.scoreSetTemp.length + 1; j++) {
                        if (this.scoreSetTemp[j]["eventID"] === -99) {
                            return j - i
                        }
                    }
                })();
    ...
    render() {
        return (
            <div>
                <div style={{opacity: (this.state.hoverShow ? '1' : '0')}}
                     onMouseEnter={() => this.setState({hoverShow: true})}
                     onMouseLeave={() => this.setState({hoverShow: false})}>
                    <Button color="success" id="startNew" onClick={this.handleMouseDown}>New Participant</Button>
                    &nbsp;
                    <Button color="primary" id={intervalCPositionBroken[0]["scoreIDformatted"]}
                            onClick={this.handleMouseDown}>{intervalCPositionBroken[0]["scoreID"]}</Button>
                    &nbsp;
                    <Button color="primary" id={singleNoteCPosition[0]["scoreIDformatted"]}
                            onClick={this.handleMouseDown}>{singleNoteCPosition[0]["scoreID"]}</Button>
                    &nbsp;
                    <Button color="primary" id={singleNoteGPosition[0]["scoreIDformatted"]}
                            onClick={this.handleMouseDown}>{singleNoteGPosition[0]["scoreID"]}</Button>
                    &nbsp;
                    <Button color="secondary" id="resetCurrent" onClick={this.handleMouseDown}>Reset Current
                        Round</Button>
                    <br/>
                ...
                <div>
                    <CreateScoreWithBlanksLC hoverShow={this.state.hoverShow} participantID={this.state.participantID}
                                             scoreSet={this.state.scoreSet} ptKeyName={this.state.ptKeyName}
                                             loopLocation={this.state.loopLocation} loopLength={this.state.loopLength}
                                             reset={this.reset} midiEvent={this.state.midiEvent}/>
                    <EventRecorder hoverShow={this.state.hoverShow} participantID={this.state.participantID}
                                   scoreSet={this.state.scoreSet} ptKeyName={this.state.ptKeyName}
                                   loopLocation={this.state.loopLocation} loopLength={this.state.loopLength}
                                   reset={this.reset} midiEvent={this.state.midiEvent}/>
                    {console.log("pass")}
                ...
                <div style={{
                    ...
                    <form>
                        <label>
                            ...
                            <input id="eee" type="text" value={this.state.inputValue}
